...
- **2026-07-28 — NIGHTFANG SET** (Inspiration Vault · Sets, $185 USD): coordinated panther ring + bangle, black enamel-finished metal · black synthetic stones · green synthetic emerald eyes. Sold only as complete set. Mirrors STAMPEDE architecture exactly (`useAddToCart`, `LuxuryMotion`, no size selector, single SKU `IV-NIGHTFANG-SET`). Route: `/inspiration-vault/nightfang-set` + shortcut `/nightfang-set`. Placed at top of `VAULT_PIECES` in `InspirationVaultPage.jsx`. Gallery: hero (marble two-piece) → on-body → 4 ring detail angles → 4 silent product motion videos (all autoplay, muted, loop). Assets in `/app/frontend/public/inspiration-vault/nightfang-set/`. Explicit Fine Jewelry disclaimer near the details block. **Zero changes to STAMPEDE, catalog.py, checkout.py, stripe_routes.py, or the SCACCO MATTO Stripe pilot** (verified via `git diff --stat`).
...
## SCACCO MATTO — Hero Video + White Gold release (2026-07-26)
- **Hero video** — supplied `XiaoYing_Video_1785086551909.mp4` saved (audio stripped via `ffmpeg -an`) to `/fine-jewelry/scacco-matto/hero-video.mp4`. Mounted at the top of the SCACCO MATTO page inside a 16:9 aspect wrapper with `object-fit:contain` (no cropping). Attributes: `autoPlay, loop, muted, playsInline, controls={false}, disablePictureInPicture, preload='metadata'`. Ref-based `defaultMuted=true, volume=0, muted=true, playsInline=true` also applied on mount. `aria-label="SCACCO MATTO yellow-gold sapphire ring campaign film"`. Poster fallback = `/fine-jewelry/scacco-matto/hero-three-quarter.png`. `prefers-reduced-motion: reduce` renders the poster image only (no `<video>` mount). Scoped to `/scacco-matto` route only — video does NOT appear on the Fine Jewelry shop card, Inspiration Vault cards, or the homepage.
- **White Gold released** — all `disabled`, `aria-disabled`, `tabIndex=-1`, keyboard/pointer/`chooseColour` blockers, and `COMING SOON` text removed. Both Yellow Gold and White Gold pills are now equally selectable.
- **USD price map (unchanged, now fully live):** 10K Yellow $8,100 / 14K Yellow $8,500 / 10K White $8,300 / 14K White $8,700. `unit_amount_cents`: 810000 / 850000 / 830000 / 870000. Currency: USD.
- **Colour-based gallery switching** — Yellow Gold shows the 8-slot yellow gallery; White Gold shows a **4-slot** white-gold gallery (hero-three-quarter, elevated-angle, workbench-editorial, rear-architecture). 3 additional white slots per the spec (yellow-circular-blue-square macro, front gemstone detail, lifestyle-hand) were NOT supplied in the artifact upload — the gallery cleanly displays what's available with no stand-ins. `useEffect` triggers `setIdx(0)` on either `karat` OR `colour` change (never mixes yellow + white).
- **SKUs**: `SM-{10K|14K}-{YELLOW|WHITE}-{SIZE}`. Cart variant string uses the correct colour label (e.g. `"14K · White Gold · US 10"`).
- **Description copy** updated to: "Available in 10K and 14K yellow or white gold."
- Verified live: hero video autoplays silently (paused=false-ish outside headless), no controls, no PiP, gallery switches without page reload, karat+colour reset the gallery to slot 1, add-to-cart writes correct SKU/variant/karat/metalColour/currency to `localStorage.phileon_cart` for every combination tested (14K White US 10 verified as canonical example).
