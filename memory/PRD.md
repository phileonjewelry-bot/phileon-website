...
### Feb 2026
- **[DONE Feb 11]** **CRESTA NERA Hinged Bangle unblocked (wave 5)** — Merchant-signed 2 metals × 4 wrist sizes trusted matrix (`10K/14K Yellow Gold` × `Small 180 mm / Medium 190 / Large 200 / Extra Large 210`). 10K: $10,495–$11,245 USD · 14K: $11,795–$12,545 USD. Backend adds `_resolve_cresta_nera()` + `CRESTA_NERA_MATRIX`. `checkout.py::CartItemIn` gains `wristSize` field. Frontend `CrestaNeraBanglePage.jsx` PRICE_MATRIX synced to merchant matrix + cart payload now transmits `variant` (metal id) and `wristSizeId` for trusted server validation. SKU pattern: `CRESTA-{10K|14K}-{S|M|L|XL}`. **82 products** trusted.
