...
## CHANGELOG
- 2026-02-12 — **BOSS KNOT configurator simplified to 2 metals**: Sterling Silver $1,950 + 10K White Gold $4,200 (10K Yellow Gold removed). Flat 2-button configurator (no cascading colour/karat). **Dynamic hero + gallery**: default landing shows gold marquee hero + 9-frame gold gallery; selecting Silver or White Gold swaps to silver hero (archive-10) + 5-frame silver gallery. Add-to-Cart disabled until metal is chosen ("SELECT A METAL"). All prices synced in `livePricingConfig.js`, `pricing_engine.py`, `products.js`, `ShopDropPage.jsx`, `HomePage.jsx`. `/api/validate-cart` returns diff=0 for both SKUs.
